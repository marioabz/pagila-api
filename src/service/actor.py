...
def get_actors_paginated(
    ...
    return {"items": actors, "next_cursor": actors[-1].actor_id if actors else None}


def get_movies_by_actor(db: Session, actor_full_name: str, sort_desc: bool = True):
    first_name, last_name = actor_full_name.split(" ")
    sorting = Film.release_year.desc() if sort_desc else Film.release_year.asc()
    stmtn = (
        select(Film.title, Film.release_year, Film.rental_rate, Film.rating)
